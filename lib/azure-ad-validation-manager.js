...
function AzureActiveDirectoryValidationManager() {
    ...
    /*
     * Extracts the tenant id from the given jwt token
     */
    ...
    /*
     * Extracts the version of the token from the given jwt token
     */
    self.getVersion = function (jwtString) {
        var decodedToken = jsonwebtoken.decode(jwtString);

        if (decodedToken) {
            return decodedToken.ver;
        } else {
            return null;
        }
    };
